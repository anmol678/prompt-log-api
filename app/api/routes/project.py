...
async def update_project(*, db: Session = Depends(dependencies.get_db), project_in: ProjectCreate):
...
def get_projects(*, db: Session = Depends(dependencies.get_db)) -> list[Project]:
    projects = crud_project.get_multi(db=db)
    return projects


@router.get("/project/{id}", response_model=Project)
def get_project(*, db: Session = Depends(dependencies.get_db), id: int) -> Project:
